...
# Only the three types evidenced against the authentic APP feed are ingested.
# VO/DR must not abort the other types because their APP schema is unverified.
ACTIVE_TYPES = frozenset({"TC", "FL", "WF"})
...
def parse(raw: bytes, now: dt.datetime, entry: dict) -> FetchResult:
    ...
    for feature in doc["features"]:
        ...
        if feature["properties"].get("eventtype") not in ACTIVE_TYPES:
            continue  # EQ/TS absichtlich ausgeschlossen; VO/DR App-Schema noch unbestätigt.
