...
- Der bestehende App-Feed-Adapter übernimmt weiterhin `TC` (Zyklon), `FL` (Flut) und `WF` (Waldbrand). `VO` (Vulkan) und `DR` (Dürre) laufen jetzt **als getrennte öffentliche Quellen** (`gdacs-volcano`, `gdacs-drought`) über die offiziellen typ-spezifischen GeoJSON-Feeds. Dadurch bleiben ihre abweichenden Feldformate und Zustandsregeln isoliert. `EQ` und `TS` werden im GDACS-App-Adapter weiterhin ignoriert. Alle GDACS-Warnstufen bleiben in der Quellskala `Green`, `Orange`, `Red`, ohne Vergleich zur USGS-Skala.
...
Die Quelle veröffentlicht keine `metadata.count` wie USGS. Zudem schließt GDACS selbst Vollständigkeitsgarantien aus. Der Adapter liefert deshalb **immer `complete=false`**: Ein fehlendes Ereignis bedeutet weder zurückgezogen noch veraltet, und eine Antwort mit `features:[]` beweist keinen leeren GDACS-Bestand. Dieses Verhalten verhindert Löschungen durch einen gekürzten Feed, führt aber ohne gesonderte Aufbewahrungsregel langfristig zu anwachsenden Altbeständen. Erfolgreiche Abrufe mit **null relevanten Einträgen** werden deshalb separat zeitlich beobachtet: `empty_since` läuft auch bei `complete=false`. Hält dieser Zustand länger als **72 Stunden** an (`max_empty_h:72`), wird ausschließlich `fetch_health=degraded` gesetzt. `data_state` bleibt unverändert; ConflictWatch behauptet damit ausdrücklich **nicht**, GDACS sei leer. Sobald ein erfolgreicher Abruf wieder relevante Einträge enthält, wird die Leer-Uhr zurückgesetzt. Nach jedem **erfolgreich geparsten** GDACS-Abruf läuft ein nicht erneut gesichtetes Ereignis nach mehr als 30 Tagen seit `ingest.last_seen_at` technisch aus dem aktiven Bestand aus (`unseen_expiry_days:30`); es wird NICHT als beendet oder zurückgezogen gemeldet. Fehlerhafte oder übersprungene Abrufe löschen nichts. `retention_days:30` ist weiterhin Schema-Pflicht und wirkt bei `complete=false` nicht.

Die drei Fixtures `real_2026-09-20_{tc,fl,wf}_excerpt.json` enthalten je **ein unverändertes echtes Feature** aus dem offiziellen App-Feed. Die `FeatureCollection`-Hülle wurde für den Test neu gebildet; diese Ausschnitte sind **keine vollständige Originalantwort**. Über die branchbezogene GitHub Action gelang am 20.09.2026 um 11:47 UTC ein vollständiger Abruf: **134475 Bytes**, SHA-256 `3e7361a1aeb6ab71c44c8bff1be92ae7f33466860c2e263542c3de90c3c000cb`, **100 Features** (EQ 22, TC 3, FL 3, WF 72), **78** als relevant verarbeitet. [Lauf und Rohdaten-Artefakt](https://github.com/bene901/conflictwatch/actions/runs/35508798542).

Am **21.09.2026** wurden VO und DR separat gegen die offiziellen GDACS-Endpunkte geprüft. Der aktuelle Dürre-Feed `gdacsDR.geojson` lieferte **26 Features**; pro Ereignis treten ein `Point_Centroid` und ggf. zusätzliche Flächengeometrien auf. ConflictWatch übernimmt ausschließlich den Quell-Zentroid und erzeugt aus dem Polygon **keine zweite Meldung**. Der aktuelle VO-Bulkfeed war zu diesem Zeitpunkt legitim leer; zusätzlich wurde ein echter 2026er Vulkanfall (Fuego, Guatemala, Event `1000145`) über die offizielle GDACS-Event-API geprüft. Die unveränderten Belegfeatures liegen als `real_2026-09-21_dr_centroid.json` und `real_2026-09-21_vo_event.json` in den Tests. GDACS dokumentiert die Event-API und die typ-spezifischen GeoJSON-Ausgaben in seinem API-Quickstart/Swagger.
...
Weiterhin offen: wiederholte zeitlich getrennte VO/DR-Rohantworten und reale Episodenwechsel. Ein leerer VO-Feed ist ausdrücklich **keine Entwarnung** und löst deshalb keinen Leer-Alarm aus; der DR-Feed wird dagegen auf ungewöhnlich lange erfolgreiche Leere überwacht.
...
Die Karte hat getrennte Filter für Waldbrände, Überschwemmungen, Wirbelstürme, Vulkane und Dürren; angeboten wird nur, was im Datenstand vorkommt. Standardmäßig zeigt sie ausschließlich Ereignisse aus dem **jüngsten erfolgreichen Abruf** (dieselbe Regel wie die Liste, siehe oben). Ältere, noch gespeicherte Meldungen sind über einen eigenen Filter einblendbar, werden dann abgeschwächt dargestellt und tragen den Hinweis, dass sie nicht aus dem jüngsten Abruf stammen — ausdrücklich keine Entwarnung.
